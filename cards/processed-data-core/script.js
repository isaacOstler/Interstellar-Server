...
Interstellar.addCoreWidget("Processed Data",function(){
	...
	*/
	...
	Interstellar.onPresetValueChange("processedData.presets",function(fetchedPresets){
		if(fetchedPresets == null){
			//presetEditor.js will init defualt values here
			return;
		}
	...
	*/
	...
	presetDropdownPresetSelect.on("input change click",function(event){
		//get the index of the selected preset
		var selectedPresetTitle = Number(presetDropdownPresetSelect[0].selectedIndex);
	...
	presetDropdownMissionSelect.on("input change click",function(event){
		//get the index of the selected preset
		var selectedMission = Number(presetDropdownMissionSelect[0].selectedIndex);
		//get the preset value
		currentProcessedDataPreset = presets[selectedMission];
		//the value of the processed data textbox
		initPresetValues();
	});
	...
	editPresetsButton.click(function(event){
		Interstellar.openCoreWindow("processedDataSettingsWindow",event);
	});
});