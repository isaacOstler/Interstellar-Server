    ...
    for(i = 0;i < renderedContacts.length;i++){
        ...
        if(renderedContacts[i].type == "contact"){
            ...
            if(contact == undefined ){
                ...
                if(!textureFound){
                    var newTextureCache = 
                    {
                        ...
                        "map" : new THREE.TextureLoader().load("/resource?path=public/" + renderedContacts[i].icon)
                    }
            ...
            if(contact == undefined ){
                ...
                var texture = new THREE.TextureLoader().load("/resource?path=public/" + renderedContacts[i].icon);
                //now we need to make a material with that texture
                var material = new THREE.MeshBasicMaterial( { map: texture,transparent: true } );
                //now make the actual mesh
                var newContact = new THREE.Mesh(geometry, material);
