var securityTrackingHasInit = false;
Interstellar.addCoreWidget("Security Tracking",function(){
	if(securityTrackingHasInit){
		return;
	}
	securityTrackingHasInit = true;

	//DOM References
	var canvas = $("#Security-Tracking_canvas"),
	...
	*/
	...
	$.getJSON('/resource?path=public/deckData.json&screen=security-tracking-core', function(deckDataJSONFile) {
		...
		$.getJSON('/resource?path=public/randomNames.json&screen=security-tracking-core',function(jsonLoaded){
	...
	setInterval(function(){
		for(var i = 0;i < officerPositions.length;i++){
			if(Math.random() > .98){
				...
				sendOfficerToRoom(i,zones[randomZone].zoneName,zones[randomZone].zoneDeck);
			}
		}
		Interstellar.setDatabaseValue("securityTracking.officerPositions",officerPositions);
	},5000);
